...
export const getServerSideProps = withI18n(['jobcard'])(async (ctx) => {
  ...
  const [catalogueRows, tierRows, promoRows] = await Promise.all([
    ...
    prisma.promo.findMany({ where: { group_id: user.group_id, active: true }, orderBy: { code: 'asc' }, select: { id: true, code: true, label: true, promo_type: true, amount: true, targets: { select: { item: { select: { id: true, title: true, name: true } } } } } }) as Promise<any[]>,
  ]);
