...
// THE WORKSHOP LEDGER RULE lives with every other series rule (lib/invoice-series-scope), where a
// fifth series cannot compile until it has been answered for every question at once.
import { IN_WORKSHOP_LEDGER, seriesWhere } from '@/lib/invoice-series-scope';
...
export function fetchLedgerInvoices(ctx: { groupId: string; siteIds: string[]; from: Date; to: Date }): Promise<LedgerInvoice[]> {
  return prisma.invoice.findMany({
    where: { group_id: ctx.groupId, site_id: { in: ctx.siteIds }, ...seriesWhere(IN_WORKSHOP_LEDGER), ...notVoided, ...effectiveIssueDateWhere(ctx.from, ctx.to) },
