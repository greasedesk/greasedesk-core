...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const rows = await prisma.jobCardPhoto.findMany({
      ...
      select: { id: true, stage: true, slot: true, label: true, r2_key: true, uploaded_at: true, user: { select: { name: true } } },
    });
  ...
  if (req.method === 'POST') {
    const { jobCardId, stage, slot, label, photoId, key } = (req.body || {}) as any;
    ...
    const row = await prisma.jobCardPhoto.create({
      data: { id: photoId, job_card_id: jobCardId, group_id: ctx.user.group_id, stage, slot, label: label ? String(label).slice(0, 200) : null, r2_key: key, uploaded_by: ctx.user.id },
      select: { id: true },
    });
