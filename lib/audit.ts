...
export type AuditAction =
  ...
  | 'stock.boundary_declared'      // "every car sale from today is invoiced here" — stamps Group.car_sales_invoiced_from, once
  | 'stock.boundary_moved_earlier' // the declaration narrowed; never later
  ...
  | 'intake.oil_level'
  // The offer on the Intake tab, answered. Two routes to the same answer: "No thanks", or turning
  ...
  | 'quote.revised_after_agreement' // estimate changed AFTER the customer agreed — unsent revision minted for the garage to confirm: { version, grossPennies, sent:false }
