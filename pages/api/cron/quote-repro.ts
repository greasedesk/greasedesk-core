 ...
 * op=setup → throwaway ZZ tenant + owner (known pw) + site + resource + an ACCEPTED job card w/ one
 *            estimate line + ownership edge + details-done (Quote tab reachable). Returns creds + URL.
 * op=lines → the card's current JobCardItem lines (verify persistence).
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (op === 'setup') {
    ...
    const card = await prisma.jobCard.create({ data: { group_id: group.id, site_id: site.id, vehicle_id: vehicle.id, customer_id: customer.id, status: 'accepted', vat_rate: '20.00', resource_id: resource.id, start_at: new Date(Date.parse('2026-07-20T09:00:00.000Z')), booking_duration_minutes: 60, stage_details_done: true } });
    ...
    return res.status(200).json({ email, password, groupId: group.id, cardId: card.id, cardUrl: `${appBaseUrl()}/admin/jobcards/${card.id}` });
  }
