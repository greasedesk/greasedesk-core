...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (invoice.status !== 'paid') return res.status(409).json({ message: 'This invoice isn’t locked — it’s already editable.' });

  try {
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      await tx.invoiceLine.deleteMany({ where: { invoice_id: invoice.id } }); // drop the paid snapshot
      await tx.invoice.update({ where: { id: invoice.id }, data: { status: 'issued', paid_at: null } });
