...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const group = (await prisma.group.findUnique({ where: { id: groupId }, select: { fy_start_month: true, group_name: true, vat_number: true, tax_label: true } })) as { fy_start_month: number; group_name: string; vat_number: string | null; tax_label: string } | null;
  ...
  const pdf = await renderVatSummaryPdf({ ...summary, businessName: group?.group_name ?? 'Your business', vatNumber: group?.vat_number ?? null, periodLabel, currency, locale, taxLabel: group?.tax_label || 'VAT' });
