    ...
    <EngineRoomLayout role={role}>
      ...
      <div className="p-6 max-w-4xl">
        ...
        <form onSubmit={add} className="mb-6 rounded-xl border border-line bg-surface p-4 flex flex-wrap items-end gap-3">
          <div><label className="block text-xs text-muted mb-1">Country</label><input required value={country} onChange={(e) => setCountry(e.target.value.toUpperCase())} maxLength={2} className={`${input} w-20`} placeholder="GB" /></div>
          <div><label className="block text-xs text-muted mb-1">Currency</label><input required value={currency} onChange={(e) => setCurrency(e.target.value.toUpperCase())} maxLength={3} className={`${input} w-24`} placeholder="GBP" /></div>
          <div><label className="block text-xs text-muted mb-1">Tier</label>
          ...
          <div><label className="block text-xs text-muted mb-1">Amount ({currency}/month)</label><input required type="number" min="0" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className={`${input} w-32`} placeholder="35.00" /></div>
          <div><label className="block text-xs text-muted mb-1">Effective from</label><input required type="date" value={eff} onChange={(e) => setEff(e.target.value)} className={input} /></div>
          <button disabled={busy} className="bg-surface text-ink rounded-lg px-4 py-2 text-sm font-medium disabled:opacity-50">Add rate</button>
        </form>

        {loading ? <div className="text-muted text-sm">Loading…</div> : rates.length === 0 ? (
          <div className="rounded-xl border border-dashed border-line bg-surface/60 p-8 text-muted text-sm">No commission rates yet. Add the first rate above — the engine will refuse to compute commission for a country/currency/tier until a rate exists for it.</div>
        ) : (
          <div className="space-y-5">
            {[...groups.entries()].map(([k, rows]) => {
              const [cc, cur, t] = k.split('/');
              return (
                <div key={k} className="rounded-xl border border-line overflow-hidden">
                  <div className="bg-surface px-4 py-2 text-sm font-medium text-ink flex items-center gap-2">
                    <span>{cc} · {cur}</span><span className="text-muted">·</span><span className="text-muted">{TIER_LABEL[t] ?? t}</span>
                  </div>
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {rows.map((r) => (
                        <tr key={r.id} className={`border-t border-line ${r.status === 'superseded' ? 'text-muted' : ''}`}>
                          ...
                          <td className="px-4 py-2">
                            ...
                            {r.referenced && <span className="ml-2 text-[10px] text-muted">referenced</span>}
                          </td>
        ...
        <p className="mt-4 text-xs text-muted">The engine resolves a payment against the latest rate whose effective-from is on or before the payment's collection date. A frozen rate can never be edited — the remedy is always a new forward amendment.</p>
      </div>
    </EngineRoomLayout>
  );
}
